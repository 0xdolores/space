...
router.post("/playground", authMiddleware, async (req, res) => {
  try {
    ...
    if (template === "reactjs") {
      containerImage = "0xdolores/react-codespace:2";
      containerPort = 5173;
    } else if (template === "nodejs") {
      containerImage = "0xdolores/node-codespace:2";
      containerPort = 5001;
    }
    ...
    res.status(201).json({ message: "Playground created", codespace });
  } catch (err) {
...
router.post("/playgrounds/:id", authMiddleware, async (req, res) => {
  try {
    ...
    if (codespace.userId !== userId) {
      res
        .status(403)
        .json({ message: "You are not authorized to delete this playground" });
      return;
    }
